...
fn find_input_dir(this_file: &str) -> io::Result<PathBuf> {
    let srcpath = ::std::path::Path::new(this_file).parent().unwrap();
    let cwd = env::current_dir()?;
    cwd.ancestors()
        .chain(srcpath.ancestors())
        .map(|p| dbg!(p).join("input"))
        .find(|p| p.is_dir())
        .ok_or(io::Error::new(
...
fn input_file(fname: &str, part: u32, file: &str) -> io::Result<PathBuf> {
    let dir = find_input_dir(file)?;
...
pub fn get_input(fname: &str, file: &str) -> io::Result<(String, u32)> {
    let mut part = 1;
    ...
    let path = match input_path {
        Some(p) => p.into(),
        None => input_file(fname, part, file)?,
    };
...
macro_rules! evc_main {
    ...
    ( $x:expr ) => {
        fn main() -> std::io::Result<()> {
            let this_file = ::std::file!();
            let (input, part) = util::get_input($x, this_file)?;
            match part {
                1 => part1(input),
                2 => part2(input),
                3 => part3(input),
